...
def load_data_from_yadisk(public_link):
    try:
        # 1. Получаем ссылку для скачивания через API Яндекс.Диска
        base_url = 'https://cloud-api.yandex.net/v1/disk/public/resources/download?'
        final_url = base_url + urlencode(dict(public_key=public_link))
        response = requests.get(final_url)
        
        if response.status_code != 200:
            st.error(f"Ошибка Яндекс.Диска: {response.status_code} - {response.text}")
        ...
        if download_response.status_code != 200:
            st.error(f"Ошибка скачивания файла: {download_response.status_code}")
            return None, None, None, None

        # 3. Читаем данные из байтов напрямую в pandas
        import io
...
# ===== ДАЛЬШЕ ИДЁТ ВЕСЬ КОД ОТОБРАЖЕНИЯ =====
# (он остаётся точно таким же, как у вас был, без изменений)
if tasks is not None:
    ...
    st.warning("Данные не загружены. Проверьте ссылку на Яндекс.Диск и повторите попытку.")tasks, eval_list, mail, fin = load_data()
